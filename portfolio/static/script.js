...
(function initContactForm() {
  ...
  const successTxt= document.getElementById('formSuccessText');
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch('https://formsubmit.co/ajax/deepthidesetty22@gmail.com', {
        method: 'POST',
        headers: { 
      ...
      // Formsubmit relies on data.success === "true"
      if (res.ok && data.success === "true") {
        if (successTxt) successTxt.textContent = 'Message sent successfully! 🚀';
        if (success)    success.classList.add('show');
        form.reset();
      } else {
        // Handle activation error or generic error
        if (errorTxt) errorTxt.textContent = data.message || 'Make sure you activated your email with FormSubmit!';
        if (error)    error.classList.add('show');
      }
    } catch (err) {
      if (errorTxt) errorTxt.textContent = 'Network error. Please try again.';
      if (error)    error.classList.add('show');
    } finally {
...
(function initProjectCarousel() {
  ...
  function updateBtns() {
    ...
    if (nextBtn) nextBtn.disabled = current >= slides.length - getVisibleCount();
  }

  function goTo(idx) {
    current = Math.max(0, Math.min(idx, slides.length - 1));
    const slide   = slides[current];
    const offset  = slide.offsetLeft - carousel.offsetLeft;
    carousel.scrollTo({ left: offset, behavior: 'smooth' });
    updateDots();
    updateBtns();
    // Slide-in animation: give active cards full opacity
    slides.forEach((s, i) => {
      s.querySelector('.project-card').style.transition =
        'transform 0.45s cubic-bezier(0.4,0,0.2,1), box-shadow 0.45s ease, border-color 0.45s ease, opacity 0.45s ease';
      
      s.querySelector('.project-card').style.opacity = '1';
    });
  }
  ...
  function startAuto() {
    ...
    autoTimer = setInterval(() => {
      const next = current >= slides.length - getVisibleCount() ? 0 : current + 1;
      goTo(next);
    }, 4500);
  }
  ...
  window.addEventListener('resize', () => { updateBtns(); updateDots(); });
...
// ================================================================
// 23. BENTO CARD STAGGERED REVEAL
// ================================================================
(function initBentoReveal() {
  ...
  $$('.bento-card').forEach(c => obs.observe(c));
})();


// ================================================================
// 24. CERTIFICATE FLIP CARDS — click to flip on mobile
// ================================================================
(function initCertFlip() {
  $$('.cert-flip').forEach(card => {
...
// ================================================================
// 26. STAGGERED SECTION CARD REVEALS
//     Each card in a grid gets a small incremental delay
// ================================================================
(function initStaggeredCards() {
  const selectors = [
    '.achieve-grid .achieve-card',
    '.cert-flip-grid .cert-flip',
